<?php
    $url = url("/proveedor/store");
    if(!isset($proveedor))$proveedor = new \App\Models\Proveedor();

    if($proveedor->exists){
        $url = url("/proveedor/update/".$proveedor->id);
    }
    if(!isset($modal))$modal = false;
?>
@include("templates.mensajes",["id_contenedor"=>"proveedor"])

{!!Form::model($proveedor,["id"=>"form-proveedor","url"=>$url])!!}
    <div class="col s12" style="padding: 20px;">
        ...
        @if(!$modal)
            <div class="col s12 center" id="contenedor-action-form-proveedor" style="margin-top: 30px;">
                <a class="btn blue-grey darken-2 waves-effect waves-light" id="btn-action-form-proveedor">Guardar</a>
                <a class="btn blue-grey darken-2 waves-effect waves-light" href="{{URL::to('/proveedor')}}">Cancelar</a>
            </div>
