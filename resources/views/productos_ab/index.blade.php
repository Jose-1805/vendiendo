...
<div class="col s12 {{$size_medium}} white" style="margin-top: 85px">
    @if(\Illuminate\Support\Facades\Auth::user()->permitirFuncion("Crear","productos","inicio") && (Auth::user()->plan()->n_productos == 0 || Auth::user()->plan()->n_productos > Auth::user()->countProductosAdministrador()))
        @if(\App\Models\Categoria::permitidos()->get()->count() !=0 && \App\Models\Unidad::unidadesPermitidas()->get()->count() !=0 && count(\App\Models\Bodega::permitidos()->get()) > 0)
    ...
    <div class="input-field col s12 hide-on-med-and-up" >
       <input type="text" name="busqueda2" id="busqueda2" placeholder="Buscar" value="{{$filtro}}">
        ...
        @if(count(\App\Models\Bodega::permitidos()->get()) <= 0)
            <p style=" background-color: #80d8ff; color: #0D47A1; padding: 10px;border-radius: 3px;">
                Para la creación de un producto se requiere que haya registrado por lo menos una bodega.
            </p>
        @endif
    ...
    <div id="contenedor-lista-productos" class="col s12 content-table-slide">
        @if(\App\Models\Categoria::permitidos()->get()->count() != 0 && \App\Models\Unidad::unidadesPermitidas()->get()->count() !=0 && (\App\Models\Proveedor::permitidos()->get()->count() !=0 || \App\Models\MateriaPrima::materiasPrimasPermitidas()->get()->count() !=0))
            @include('productos_ab.lista')
        @else
            <p style=" background-color: #80d8ff; color: #0D47A1; padding: 10px;border-radius: 3px;">Para la creación de un producto se requiere que haya registrado unidades y categorías, además de proveedores y/o materias primas según sea el caso</p>
        @endif
    </div>
