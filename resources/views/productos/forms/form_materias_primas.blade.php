    ...
    @if(\App\Models\MateriaPrima::materiasPrimasPermitidas()->get()->count() > 0)
        ...
        <div class="col s12">
            ...
            <?php
                ...
                $checked = "";
                if(isset($producto) && $producto->omitir_stock_mp == 'si'){
                    $checked = 'checked="checked"';
                }
            ?>
            <p>
                <input type="checkbox" name="omitir_stock_mp" id="omitir_stock_mp" value="si" {{$disabled}} {{$checked}}/>
                <label for="omitir_stock_mp">Descontar materias primas en facturación (no se inicializa stock en este producto).</label>
            </p>
        </div>
</div>
@else
