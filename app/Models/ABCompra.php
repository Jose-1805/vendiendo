...
class ABCompra extends Model {
    ...
    public static function permitidos(){
        ...
        if($perfil->nombre == 'superadministrador'){
            return ABCompra::whereNotNull('id');
        }else {
            return ABCompra::where("compras.usuario_id",$user->userAdminId());
        }
    }
    function abonos(){
        return $this->hasMany("App\Models\ABAbono","tipo_abono_id")->where("tipo_abono","compra");
    }
    ...
    public function historialCostos(){
        return $this->belongsToMany("App\Models\ABHistorialCosto","compras_historial_costos","compra_id","historial_costo_id");
    }

    public function materiasPrimasHistorial(){
        return $this->belongsToMany(MateriaPrimaHistorial::class,"vendiendo.compras_materias_primas_historial");
    }
    ...
    public static function ultimaCompra(){
        return ABCompra::permitidos()->orderBy("id","DESC")->orderBy("created_at","DESC")->orderBy("numero","DESC")->first();
    }
    
    public function updateCompra($id_compra,$valor_devolucion){
        $compra = ABCompra::permitidos()->find($id_compra);
        if ($compra){
            $valor_compra = $compra->valor;
            $valor_compra = $valor_compra - $valor_devolucion;

            $compra = ABCompra::permitidos()->where('id',$id_compra)
                ->update([
                    'valor'=>$valor_compra
                ]);
            return $compra;
    ...
    public function updateStock($id,$cantidad_devolucion,$tipo_compra){
        if ($tipo_compra == "MateriaPrima")
            ...
            $elemento = ABProducto::permitidos()->find($id);
    ...
    public static function calculateQuantity($tipo_compra,$id_compra,$id_elemento,$cantidad_devolucion,$id_elemento_historial){
        ...
        if ($tipo_compra == "MateriaPrima"){
            $compra_elemento = DB::table('vendiendo.compras_materias_primas_historial')->select('vendiendo.compras_materias_primas_historial.cantidad','vendiendo.compras_materias_primas_historial.id')
                ->join("vendiendo.materias_primas_historial","vendiendo.compras_materias_primas_historial.materia_prima_historial_id","=","vendiendo.materias_primas_historial.id")
                ->where('compra_id',$id_compra)
                ->where('vendiendo.materias_primas_historial.materia_prima_id',$id_elemento)->first();
        }else if($tipo_compra == "Producto"){
            $compra_elemento = DB::table('vendiendo_alm.compras_historial_costos')->select('cantidad','id')
                ->where('compra_id',$id_compra)
                ->where('historial_costo_id',$id_elemento_historial)->first();
        }
    ...
    public static function deleteElemento($tipo_compra,$id_compra,$id_elemento){
        ...
        if ($tipo_compra == "MateriaPrima"){
            ...
            $compra_elemento = DB::table('compras_historial_costos')->select('cantidad','id')
                ->where('compra_id',$id_compra)
                ->where('historial_costo_id',$id_elemento)->first();
        }

        if ($tipo_compra == "MateriaPrima")
            $elemento = DB::table('compras_materias_primas_historial')->where('id',$compra_elemento->id)->delete();
        else if($tipo_compra == "Producto")
            $elemento = DB::table('compras_historial_costos')->where('id',$compra_elemento->id)->delete();
    ...
    public static function updatePaymentStatus($id_compra){
        ...
        if ($compra){
            $compra = ABCompra::where('id',$id_compra)
                ->update([
                    'estado_pago'=>"Pagada"
                ]);
            return $compra;
    ...
    public static function getAbonosByProveedor($proveedor_id){
        $compras_proveedor = ABCompra::permitidos()->select('id')
    ...
    public static function getTotalAbonos(){
        $compras = ABCompra::permitidos()->select('id')
            ->where('estado_pago','Pendiente por pagar')
            ->get();
       $abonos=0;
       $total_abonos  = 0;
    ...
    public static function getValorEgresosByFecha($fecha_caja){
        $compras = ABCompra::permitidos()->whereDate('updated_at', '=', $fecha_caja);
