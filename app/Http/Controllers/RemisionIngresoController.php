...
class RemisionIngresoController extends Controller
{
	public function __construct()
	{
		$this->middleware("auth");
		$this->middleware("modConfiguracion");
		$this->middleware("modRemisionIn");
	...
	{
		return view('remisiones_ingreso.index')->with("display_factura_abierta",false);
	}

    public function getListRemisionesIngreso(Request $request)
    {
        // Datos de DATATABLE
		...
		if($orderBy == "numero")$orderBy = "remisiones_ingreso.id";

		if(strtoupper($sortColumnDir) == "ASC")$sortColumnDir = "DESC";
		else $sortColumnDir = "ASC";

		if($orderBy == "created_at")$orderBy = "remisiones_ingreso.created_at";
		if($orderBy == "usuario_creador")$orderBy = "vendiendo.usuarios.nombres";


        $remisiones_ingreso = RemisionIngreso::permitidos()->select("remisiones_ingreso.*");

        $remisiones_ingreso = $remisiones_ingreso->join("vendiendo.usuarios","remisiones_ingreso.usuario_creador_id","=","vendiendo.usuarios.id");

        $remisiones_ingreso = $remisiones_ingreso->orderBy($orderBy, $sortColumnDir);
        $totalRegistros = $remisiones_ingreso->count();
        //BUSCAR
        if ($search['value'] != null) {
            $remisiones_ingreso = $remisiones_ingreso->whereRaw(
                " ( remisiones_ingreso.id LIKE '%" . $search["value"] . "%' OR " .
                " remisiones_ingreso.numero LIKE '%" . $search["value"] . "%' OR" .
                " remisiones_ingreso.created_at LIKE '%" . $search["value"] . "%' OR" .
                " LOWER(CONCAT(vendiendo.usuarios.nombres, ' ', vendiendo.usuarios.apellidos)) LIKE '%" . \strtolower($search["value"]) . "%')");
        }

        $parcialRegistros = $remisiones_ingreso->count();
        $remisiones_ingreso = $remisiones_ingreso->skip($start)->take($length);

        $object = new \stdClass();
        if($parcialRegistros > 0){
            foreach ($remisiones_ingreso->get() as $value) {
        ...
        $data = ['length'=> $length,
            ...
            'last_query' => $remisiones_ingreso->toSql(),
            'recordsTotal' =>$totalRegistros,
            'recordsFiltered' =>$parcialRegistros,
            'data' => $myArray,
            'info' =>$remisiones_ingreso->get()];
	...
	public function postStore(Request $request){
		if(Auth::user()->permitirFuncion("Crear","Remision ingreso","inicio")) {
            if(Auth::user()->bodegas == 'si')
                return response(['error'=>['Unauthorized.']],401);

                    $ultimaRemision = RemisionIngreso::ultimaRemisionIngreso();
                    $numero = 1;
                    $productos = false;
                    ...
                    if($materias_primas || $productos) {
                        DB::commit();
                        Session::flash("mensaje", "La remisión de ingreso ha sido registrada con éxito");
                        return ["success" => true];
	...
	public function getCreate()
	{
		...
		if (Auth::user()->permitirFuncion("Crear", "Remision ingreso", "inicio"))
            if(Auth::user()->bodegas == 'no')
                return view('remisiones_ingreso.create')->with("display_factura_abierta", false);
	...
	{
	    $remision = RemisionIngreso::permitidos()->where("id", $id)->first();
		if ($remision) {
			$user = Auth::user()->userAdminId();
			return view('remisiones_ingreso.detalle')->with("remision", $remision);
		}
		return redirect("/remision-ingreso");
	}
    ...
    public function postListaElementosRemisionIngreso(Request $request){
		if($request->has("tipo_elemento")){
            ...
            if($request->has("filtro"))$filtro = $request->input("filtro");
                if ($request->input("tipo_elemento") == "producto") {
                    ...
                    return view("remisiones_ingreso.lista_productos")->with("tipo",$tipo);
                } else if ($request->input("tipo_elemento") == "materia prima") {
                    return view("remisiones_ingreso.lista_materias_primas");
