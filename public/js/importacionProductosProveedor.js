...
function importar(){
    ...
    if (input.files && input.files[0]) {
        ...
        var url = $("#base_url").val() + "/productos-proveedor/store-importacion";
...
function procesarProducto(){
    ...
    if(error) {
        ...
        var params = {_token:$("#general-token").val(),id:id_procesar,categoria:categoria,unidad:unidad};
        var url = $("#base_url").val()+"/productos-proveedor/procesar-importacion";
        id_procesar = null;
...
function rechazarProducto() {
    ...
    var url = $("#base_url").val()+"/productos-proveedor/rechazar-importacion";
    id_rechazar = null;
    $.post(url,params,function(data){
        if(data.success){
            $(item_select).parent().parent().parent().remove();
