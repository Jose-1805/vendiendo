...
$(function(){
    ...
    $('.cantidad_almacen').keyup(function (e) {
        var cantidad_permitida = 0;
        var bodegas = 0;

        if($('#Terminado').is(':checked')) {
            $('#contenedor-proveedores-productos .cantidad').each(function (i, el) {
                cantidad_permitida += parseInt($(el).val());
            })
        }else {
            cantidad_permitida += $("#stock").val();
        }

        bodegas = cantidad_permitida;

        var cantidad_ingresada = 0;
        $('.cantidad_almacen').each(function (i,el) {
            cantidad_ingresada += parseInt($(el).val());
        })

        if(cantidad_ingresada > cantidad_permitida){
            $(this).val($(this).val() - (cantidad_ingresada - cantidad_permitida));
            bodegas = 0;
        }else{
            bodegas -= cantidad_ingresada;
        }

        $('#cantidad_bodega').html(bodegas);
    })

    $("#btn-action-form-producto").click(function(){
        var action = $("#form-producto").attr("action").split('/')[$("#form-producto").attr("action").split('/').length-1];
        ...
        if(action == 'store'){
            ...
            }else {//si es un producto compuesto las cantidades se cuentan desde otro form
                cantidad += $("#stock").val();
            }
            if(cantidad > 0) {
                continuar = false;
                $('#cantidad_bodega').html(cantidad);
                $('.cantidad_almacen').val(0);
                $('#modal-almacenes-stock').openModal();
            }
        }
    ...
    $('#btn-almacenes-stock').click(function () {
        action_producto();
    })
...
function cargarTablaProductos() {
    ...
    ProductosTabla = $('#ProductosTabla').DataTable({
        ...
        "createdRow": function (row, data, index) {
            $('td', row).eq(7).html(data.descripcion.length > 20 ? data.descripcion.substring(0, 20) + '...' : data.descripcion.substring(0, 20));
            ...
            if(data.tipo_producto == "Compuesto"){ $('td', row).eq(1).html("<div class='col s12 center tooltipped' data-tooltip='Producto "+ data.tipo_producto +"'><p><img src='img/sistema/c.png'   alt='' style='max-height: 20px;vertical-align: middle;'> <i class='fa fa-plus-circle cyan-text cambiar-stock' onclick='modalCambiarStock("+data.id+")' style='cursor:pointer;vertical-align: middle'></i> </p></div>")}
        ...
        "iDisplayLength": 5,"bLengthChange": true,"aoColumnDefs": [{ 'bSortable': false, 'aTargets': [3,4,7] }],
        ...
        $.post($("#base_url").val()+"/proveedor/count",{_token:$("#general-token").val()},function(data) {
            if (data == 0 || data == "0") {
                ...
                $.post(url,params,function(data){
                    ...
                    html += "<div class='input-field col s12 l3'>"+
