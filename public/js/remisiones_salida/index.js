...
$(document).ready(function(){
    cargarTablaRemisionesSalida();
});

function cargarTablaRemisionesSalida() {
    var url_remisiones = $("#base_url").val()+"/remision-salida/detalle/";
    ...
    tabla_remisiones = $('#tabla-remisiones').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": $("#base_url").val()+"/remision-salida/list-remisiones-salida",
            "type": "GET"
        },
        "columns": [
...
$(function () {
    ...
    $("#tabla-detalle-producto-remision tbody").on("click keyup","tr td .barCodeRemisionesSalida",function(){
        var tipo = $(this).parent().parent().children(".radio-tipo-elemento ").eq(0).children("p").children("input[type=radio]:checked").eq(0).val();
        if (typeof tipo == "undefined") {
            $(this).val("");
            //mostrarErrores("contenedor-errores-detalles-remision-salida", {"1": ["Seleccione el tipo de elemento  a agregar."]});
            Materialize.toast("Seleccione el tipo de elemento  a agregar.", 3000, "red")
        }
    });
    $("#tabla-detalle-producto-remision tbody").on("keyup blur","tr td .barCodeRemisionesSalida",function(){
        setTimeout(function(){
            $("#contenedor-errores-detalles-remision-salida").fadeOut(3000);
        },2000);
    ...
    $("#tabla-detalle-producto-remision tbody").on("click","tr td .nombre",function(e){
        ...
        if (e.originalEvent.type == "click"){
            barCodeProducto = $(this).parent().parent().children("td").children(".barCodeRemisionesSalida").eq(0).val();
            input_nombre = $(this);
            $(input_nombre).blur();
            ...
            if (typeof tipo == "undefined") {
                ...
                if(tipo == "producto") {
                    ...
                    $("#titulo-modal-remisiones-salida").text("Productos")
                    ...
                    $("#titulo-modal-remisiones-salida").text("Materias primas");
                    $("#contenedor-select-categorias").addClass("hide");
                }
                var load = $(input_nombre).parent().children(".fa-spin").eq(0);
                $(load).removeClass("hide");

                var url = $("#base_url").val()+"/remision-salida/lista-elementos-remision-salida";
                ...
                $.post(url,params,function(data){
                    ...
                    seleccionable = true;
                    $("#modal-elementos-remision-salida").openModal();
    ...
    $("#tabla-detalle-producto-remision tbody").on("keyup","tr td .cantidad",function () {
        var elemento = $(this).parent().parent().find('.id-pr');
        validarStock(elemento,$(this));
    });
    ...
    $("#tabla-detalle-producto-remision tbody").on("change","tr td p input[type=radio]",function () {
        ...
        $(input_nombre).parent().parent().children("td").children(".barCodeRemisionesSalida").eq(0).val("");
        $(input_nombre).val("");
...
function cargarListaElementosMp(){
    ...
    tabla_materias_primas = $('#tabla-materias-primas').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": $("#base_url").val()+"/remision-salida/list-materias-primas?"+params,
            "type": "GET"
        },
        "columns": [
...
function cargarListaProductos(){
    ...
    tabla_productos = $('#tabla-productos').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": $("#base_url").val()+"/remision-salida/list-productos?"+params,
            "type": "GET"
        },
        "columns": [
            { "data": null, "defaultContent": "", "className": "text-center" },
...
function eliminarElementoDetalleProducto(elemento,mensaje = true){
    if($("#tabla-detalle-producto-remision tbody tr").length == 1){
        if(mensaje)
            mostrarErrores("contenedor-errores-detalles-remision-salida",{"1":["No puede eliminar el elemento, una remisión debe tener por lo menos un elemento en la lista de detalles."]});
    }else {
...
function agregarElementoRemision(cero_vacios = false){
    ...
    if ((vacios == 1 && !cero_vacios) || (vacios == 0 && cero_vacios)) {
        var html = "<tr>" +
            ...
            "<td class='barCodeTd'>" +
            "<input class='barCodeRemisionesSalida' id='barCodeRemision_" + filaCompra + "' placeholder='Código de barras' onblur=\"seleccionElemento(this.id, 'barCodeProductosRemisiones')\">" +
            "</td>" +
        ...
        $("#tabla-detalle-producto-remision tbody .barCodeRemisionesSalida").eq($("#tabla-detalle-producto-remision tbody .barCodeRemisionesSalida").length - 1).focus();
        //alert($("#tabla-detalle-producto-remision tbody .barCodeRemisionesSalida").length);
    }
}
...
function seleccionElemento(barCodeRemision_ID, accion_actual, elemento_id) {
    ...
    if(seleccionable || barCodeRemision_ID) {
        ...
        if (typeof tip == "undefined" && accion_actual == "barCodeProductoRemision") {
            mostrarErrores("contenedor-errores-detalles-remision-salida", {"1": ["Selecciona el tipo de elemento a agregar."]});
            $('#contenedor-errores-detalles-remision-salida').setTimeout(function () {
                $(this).hide(1000)
            }, 3000);
            return false;
        }
    ...
    if (typeof id == "undefined" && barCodeProducto == undefined) {
        ...
        if (continuar) {
            ...
            if(id || barCodeProducto) {
                ...
                $("#progress-elementos-remision-salida-modal").removeClass("hide");
                $("#contenedor-botones-elementos-remision-salida-modal").addClass("hide");
                $.post(url, params, function (data) {
                    ...
                    if (data.success) {
                        ...
                        input_nombre = null;
                        tipo = null;
                        $("#modal-elementos-remision-salida").closeModal();
                        $(".lean-overlay").remove();
                    ...
                    $("#progress-elementos-remision-salida-modal").addClass("hide");
                    $("#contenedor-botones-elementos-remision-salida-modal").removeClass("hide");
                })
            }
        }
    }
...
function establecerCamposProducto(accion, input_ID, data){
    if(tipo == "producto"){
        ...
        if(accion == "barCodeProductosRemisiones" && barCodeProducto != undefined){
            ...
            $('#' + input_ID).parent().parent().children("td").children('.barCodeRemisionesSalida').eq(0).prop('disabled',true);

        }else {
            $(input_nombre).parent().children(".id-pr").eq(0).val(data.producto.id);
            $(input_nombre).parent().parent().children("td").children(".barCodeRemisionesSalida").eq(0).val(data.producto.barcode)
            $(input_nombre).val(data.producto.nombre);
            $(input_nombre).parent().parent().children("td").children(".cantidad").eq(0).val("1");
            $(input_nombre).parent().parent().children("td").children(".unidad").eq(0).text(data.producto.sigla);
            var iva = 0;
            ...
            $(input_nombre).parent().parent().children("td").children('.barCodeRemisionesSalida').eq(0).prop('disabled',true);
        }
        ...
        if(accion == "barCodeProductosRemisiones" && barCodeProducto != undefined){
            ...
            $('#' + input_global_ID).parent().parent().children("td").children('.barCodeRemisionesSalida').eq(0).prop('disabled',true);
        }else{
            $(input_nombre).val(data.materia_prima.nombre);
            $(input_nombre).parent().parent().children("td").children(".barCodeRemisionesSalida").eq(0).val(data.materia_prima.codigo);
            $(input_nombre).parent().children(".id-pr").eq(0).val(data.materia_prima.id);
...
function realizarRemision(){
    ...
    if($(".id-pr").eq(0).val()){
        ...
        if(!error) {
            var url = $("#base_url").val()+"/remision-salida/store";
            params._token = $("#general-token").val();
            DialogCargando("Realizando remisión de salida ...");
            $.post(url,params,function(data){
                if(data.success){
                    $("body").scrollTop(50);
                    window.location.reload(true);
                }
                $("#contenedor-boton-realizar-remision-salida").removeClass("hide");
                $("#progress-remision-salida").addClass("hide");
            }).error(function(jqXHR,error,state){
                CerrarDialogCargando();
                mostrarErrores("contenedor-errores-detalles-remision-salida",JSON.parse(jqXHR.responseText));
            })
        }
    }else{
        error = true;
    ...
    if(error){
        mostrarErrores("contenedor-errores-detalles-remision-salida",{"error":[mensaje]});
        $("body").scrollTop(100);
        $("#contenedor-boton-realizar-remision-salida").removeClass("hide");
        $("#progress-remision-salida").addClass("hide");
    }
...
function validarStock(elemento_seleccion,elemento_cantidad) {
    if($.isNumeric($(elemento_cantidad).val()) && $(elemento_seleccion).val()) {
        var tipo_elemento = $(elemento_seleccion).parent().parent().find(".radio-tipo-elemento").find("input");
        var seleccion = "";
        if (tipo_elemento.length) {
            $(tipo_elemento).each(function (i, el) {
                if ($(el).prop("checked")) {
                    seleccion = $(el).val();
                }
            })
        ...
        if (seleccion == "producto") {
            for(i in productos){
                if(productos[i].id == $(elemento_seleccion).val()){
                    if(parseFloat(productos[i].stock) < parseFloat($(elemento_cantidad).val())){
                        Materialize.toast("La cantidad máxima para el producto seleccionado es "+productos[i].stock, 7000,'red');
                        $(elemento_cantidad).val(productos[i].stock);
                    }
                }
            }
        } else if (seleccion == "materia prima") {
            for(i in materiasPrimas){
                if(materiasPrimas[i].id == $(elemento_seleccion).val()){
                    if(parseFloat(materiasPrimas[i].stock) < parseFloat($(elemento_cantidad).val())){
                        Materialize.toast("La cantidad máxima para la materia prima seleccionada es "+materiasPrimas[i].stock, 7000,'red');
                        $(elemento_cantidad).val(materiasPrimas[i].stock);
                    }
                }
            }
        }
